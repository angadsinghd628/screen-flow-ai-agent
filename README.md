...
<h1 align="center">AiFlow — 智能截图解析悬浮窗工具</h1>
...
**Ai_Flow** 是一个 Windows 桌面 AI 助手。程序启动后常驻悬浮窗，**随时输入文字与 AI 对话**；按下 `Ctrl+D` 截取屏幕任意区域，多模态大模型（豆包 VL）即刻分析图片内容，答案以**流式打字**效果显示。支持多轮上下文对话，就像和一个能"看见"你屏幕的 AI 聊天。

### 两种使用方式

| 方式 | 操作 |
|------|------|
| 💬 **纯文本对话** | 直接在悬浮窗底部输入问题 → Enter 发送 |
| 🖼️ **截图 + 提问** | `Ctrl+D` 截屏 → 弹框输入追问 → AI 结合图片回答 |
...
- 截取文档段落 → "帮我翻译/总结这段文字"
- 截取代码片段 → "这段代码有什么问题？"
- 直接提问 → "用 Python 写一个快速排序"

---

## 📸 操作演示

### 日常对话：直接在悬浮窗输入文字，Enter 发送

<img src="assets/user2.png" alt="日常对话" width="700">

> 程序启动即显示悬浮窗，底部输入框随时可用。输入任何问题，AI 流式回复。支持 Markdown 渲染、多轮上下文对话。

### 截图提问：`Ctrl+D` 选取区域 → 弹框输入追问 → AI 解析

<img src="assets/user.png" alt="截图选区" width="700">

> 全屏半透明遮罩，拖拽出矩形框，8 个锚点精细调整。按 `Enter` 确认。

<img src="assets/user1.png" alt="输入追问" width="700">

> 确认截图后弹出追问框，可输入问题或直接提交。AI 结合图片内容流式回复。
...
| 模块 | 说明 |
|------|------|
| 🪟 **常驻悬浮窗** | 启动即显示，无需截图即可文字对话，Ctrl+F 随时显隐 |
...
| 操作 | 说明 |
|------|------|
| `Ctrl+D` | 触发截图 → 弹框输入追问 → AI 回复 |
| `Ctrl+F` | 隐藏 / 显示悬浮窗 |
| 底部输入框 + `Enter` | 纯文本对话（无需截图） |
| 鼠标拖拽 | 在遮罩层上选取截图区域 |
| 拖动 8 个锚点 | 精细调整选区范围 |
| `Enter`（截图时） | 确认截图 |
| `Esc` | 取消截图 |
| 右键托盘 | 手动截图、设置 API Key、清空历史、退出 |
