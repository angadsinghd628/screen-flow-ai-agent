...
### 步骤 1：按下 `Ctrl+D`，拖拽选取截图区域

<img src="assets/user.png" alt="截图选区" width="700">

> 全屏半透明遮罩层，鼠标拖拽出矩形框，拖动 8 个锚点可精细调整范围。按 `Enter` 确认，`Esc` 取消。

### 步骤 2：输入追问（可选），AI 流式解析

<img src="assets/user1.png" alt="输入追问" width="700">

> 确认截图后可输入问题（如"这段代码有什么问题？"），也可留空直接提交。AI 逐字流式输出结果。

### 步骤 3：透明悬浮窗展示结果，支持多轮对话

<img src="assets/user2.png" alt="结果展示" width="700">

> 无边框置顶悬浮窗，Markdown 渲染，可拖拽移动 + 四角缩放。在底部输入框继续追问，AI 记住上下文。
...
| **Token 裁剪** | 自动滑动窗口裁剪历史，超过 1 轮的旧图片从上下文剔除，节省 Token |
